...
Edge device management with FlightCTL and Bootc
...
In this section we are going to build a bootc image with flightctl agent and generate an AMI and use that to provision devices in AWS

### Build base image with FlightCTL Agent
Build a base OS image with flightctl agent in it by running command below
...
We are going to test device enrollment in AWS and leverage EC2. Run an ansible playbook to launch ec2 instance as shown in command below

```sh
ansible-playbook --vault-password-file <(echo "$VAULT_SECRET") launch_instance.yaml -e @vars/device-without-microshift.yaml
```

### Injecting Agent Enrollment Credentials into Image
We are using late binding approach to inject agent credentials into the image. There are two methods I'd like to cover here.

#### Cloud Init

Sample user-data file. Im using user-data config to write the config.yaml generated by flightctl as a result of running the command to generate enrollment credentials. Cloud-Init framework on first boot will apply these configurations to the image.
...
In the ansible playbook we are going to set the user_data parameter as shown below
...
Once the EC2 instance is ready you can see a pending enrollment request in FlightCTL as shown in screen capture below
